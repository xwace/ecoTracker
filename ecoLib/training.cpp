...
namespace eco
{
	...
	*/
		...
		for (size_t i = 0; i < (size_t)num_features; i++) // for each feature
		{
			...
			for (size_t j = 0; j < fAndDel[i].size(); j++) // for each dimension
			{
				...
				else
				{
					...
					rot90(hf_conv, 3);
					cv::hconcat(fAndDel[i][j], mat_conj(hf_conv), hf_conv);  //  异常: ipp::IwException 1B0D0
				}
	...
	//************************************************************************
	void EcoTrain::train_filter(const std::vector<ECO_FEATS> &samplesf,
	...
	ECO_FEATS EcoTrain::pcg_eco_filter(const std::vector<ECO_FEATS> &samplesf,
		const std::vector<cv::Mat> &reg_filter,
		const std::vector<float> &sample_weights,

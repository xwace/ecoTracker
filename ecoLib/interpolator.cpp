...
namespace eco
{
	...
	void Interpolator::get_interp_fourier(cv::Size filter_sz,
		...
		float a)
	{
		cv::Mat temp1(filter_sz.height, 1, CV_32FC1);  // y
	//	cv::Mat temp2(1, filter_sz.width, CV_32FC1);  // x
		// [-N/2, N/2]
		...
		//	}
			...
			// 先除以最大值进行归一化，
		interp1_fs = cubic_spline_fourier(temp1 / filter_sz.height, a) / filter_sz.height;  // 三样条傅里叶变换
	//	interp2_fs = cubic_spline_fourier(temp2 / filter_sz.width, a) / filter_sz.width;
		...
		//	temp2 = temp2 / filter_sz.width;
			// std::transform的主要作用就是省去了我们自己写for循环实现
	...
	cv::Mat Interpolator::cubic_spline_fourier(cv::Mat f, float a)
	{
		...
		*/
		...
		for (int r = 0; r < bf.rows; r++)
		{
			...
			{
				bf.at<float>(r, c) = 6.0f * (1 - cos(2.0f * f.at<float>(r, c) * CV_PI))
					+ 3.0f * a * (1.0f - cos(4 * f.at<float>(r, c) * CV_PI))
					- (6.0f + a * 8.0f) * CV_PI * f.at<float>(r, c) * sin(2.0f * f.at<float>(r, c) * CV_PI)
					- 2.0f * a * CV_PI * f.at<float>(r, c) * sin(4.0f * f.at<float>(r, c) * CV_PI);
				float L = 4.0f * pow(f.at<float>(r, c) * CV_PI, 4);
				bf.at<float>(r, c) /= L;
			}
		}
		bf.at<float>(bf.rows / 2, bf.cols / 2) = 1;
